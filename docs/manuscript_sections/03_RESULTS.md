...
1. **Clinical Survival Validity:** Patients with non-positive or missing overall survival durations ($OS \le 0$ days) were excluded ($N=5$), retaining 372 cases.
2. **Transcriptomic Availability:** Primary solid tumor RNA-sequencing raw counts were required ($N=6$ excluded without primary tumor STAR counts), retaining 366 cases.
3. **Copy Number Variation Availability:** Primary solid tumor Affymetrix SNP 6.0 segment calls were required ($N=2$ excluded without segment data), retaining 364 cases.
4. **Somatic Mutation Availability:** Somatic MuTect2 variant calls were required ($N=9$ excluded without primary tumor MAF calls), retaining 355 cases.
5. **Hypermutator Phenotype Exclusion:** Patients exhibiting extreme tumor mutational burden (TMB $\ge 99$th percentile, corresponding to $\ge 513.9$ functional nonsynonymous mutations) resulting from *POLE* or *POLD1* exonuclease domain proofreading mutations were excluded ($N=4$), retaining 351 cases.
6. **Transcriptomic Sequencing Failure Exclusion:** One patient displaying extreme transcriptomic dropout (detection fraction = 38.45%, below the pre-specified 40.0% threshold) was excluded ($N=1$), yielding a locked primary modeling cohort of 350 patients.
...
- **Copy Number Variation (CNV):** Gene-level $\log_2$ copy ratios derived from precomputed GDC Affymetrix SNP 6.0 segment files (primary tumor aliquots / Center 01, mapped to canonical autosomal GRCh38/hg38 coordinates via length-weighted mean of overlapping segment means) were ranked by sample variance across outer training samples, and the top 5,000 genes were retained.
